    ...
    if cin == 'y':
        ...
        while not quit_flag:
            ...
            if coins < 5:
                recharge_flag = False
                # 是否充值
                while not recharge_flag and not quit_flag:
                    cin = input("当前金币为0,金币一个0.5元,是否充值(y/n):")
                    if cin == 'y':
                        # 是否为10的整数倍
                        while not recharge_flag:
                            ...
                            if recharge_money.isdigit():
                                ...
                                if recharge_money % 10 == 0:
                                    value_coins = recharge_money // 0.5
                                    coins += value_coins
                                    recharge_flag = True
                        ...
                        print("输入有误,请重新输入!")
            # 金币不足且未充值
            if coins < 5 and quit_flag:
                break
            # 游戏开始
            total += 1
            # 玩游戏消耗5个金币
            coins -= 5
            print("游戏开始!本局游戏消耗5枚金币!当前金币数为:%d" % coins)
            coins += 1
